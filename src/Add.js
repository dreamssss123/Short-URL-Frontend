...
function Edit() {
    ...
    const handleSubmit = async (e) => {
        ...
        await fetch(process.env.REACT_APP_BACKEND_URL+'/api/url/create', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        .then(data => {
            console.log(data);
            if( data.status===true ){
                navigate(url, { state: { message: 'Edit Success', type: 'success' } });
            }else{
                setAlertz(true);
            }
        });
    ...
    return (
        <div style={login_box}>
            ...
            <div className='container mx-auto'>
                <h1 className='text-center mt-7 text-3xl font-bold underline mb-5'>Add</h1>
                <form onSubmit={handleSubmit}>
                    <input type='text' name='textz' className="border-2 border-black pt-1 pb-1 pl-3 pr-3" style={input_textz} />
                    <button typee="button" className="border-2 border-black pt-1 pb-1 pl-5 pr-5">Create</button>
                </form>

                <br />
                <Link to={url} className="border-2 border-black py-0 px-5 hover:bg-blue-300 inline-block">Back</Link>
            </div>
        </div>
    )
}
